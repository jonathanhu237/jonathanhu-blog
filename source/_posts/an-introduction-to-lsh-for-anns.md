...
tags:
    - LSH
    - ANNS
cover: /img/covers/cosmic_nebula.png
thumbnail: /img/covers/cosmic_nebula.png
---
...
Mathematically, given a dataset $\mathcal{O}=\{o_1,o_2,...,o_n\}$, where $p_i$ represents a point in $d$-dimensional space, the goal of NNS is to find a point $o^{\ast} \in\mathcal{O}$ such that:

$$
\text{dist}(o^{\ast},q)=\min_{o_i\in\mathcal{O}}\text{dist}(o_i,q)
$$

where $q\in\mathbb{R}^{d}$ is the query point, and the $\mathrm{dist}(\cdot,\cdot)$ is a distance metric.
...
In the context of ANNS, the goal is to find a point $\hat{o}$ from the dataset $\mathcal{O}$ that is approximately the nearest neighbor to a query point $q$. Instead of minimizing the exact distance, the objective is to find $\hat{o}$ such that the distance between $q$ and $\hat{o}$ is within some approximation factor $c$ of the true nearest neighbor distance.

$$
\mathrm{dist}(\hat{o},q)\le c\cdot\mathrm{dist}(o^\ast, q)=c\cdot\min_{o_i\in\mathcal{O}}\mathrm{dist}(o_i,q)
$$

where $c\ge 1$ is the approximation ratio that controls the trade-off between accuracy and computational efficiency. Since the ANNS problem is highly related to the parameter $c$, it can also be referred to the $c$-NNS problem, and $\hat{o}$ can be referred to as the $c$-NN of $q$.
...
Given a dataset $\mathcal{O}\subset\mathbb{R}^d$, a query point $q\in\mathbb{R}^d$, a distance threshold $R>0$, and a approximation ratio $c\ge 1$, the approximate **near** neighbor search problem aims to design a data structure that satisfies the following conditions:

- If there exists $o_{i}\in\mathcal{O}$ satisfying $\text{dist}(o_i,q)\le r$, the data structure should return YES and a point $\tilde{o}\in\mathcal{O}$ satisfying $\text{dist}(\tilde{o},q)\le cr$,
- If for all $o_i\in\mathcal{P}$, $\text{dist}(o_i,q)\gt cr$ , the data structure should return NO.
- Otherwise, the result is undefined.

The approximate near neighbor search problem is strongly influenced by the parameters $c$ and $R$, and it is often referred to as the $(R,c)$-NNS problem. In this context, the point $\tilde{o}$ is called the $(R,c)$-NN of the query.
...
Given a dataset $\mathcal{O}\in\mathbb{R}^d$ and a distance metric $\text{dist}(\cdot,\cdot)$ associated with a distance measure $D$, a family of hash family $H$ is called $(r_1,r_2,p_1,p_2)$-sensitive for $D$ if, for any pair of point $o_i,o_j\in\mathcal{P}$ and any $h\in\mathcal{H}$, the following conditions hold:

-   if $\text{dist}(o_i,o_j)\le r_1$, then $\Pr[h(o_i)=h(o_j)]\ge p_1$
-   if $\text{dist}(o_i,o_j)\gt r_2$, then $\Pr[h(o_i)=h(o_j)]\le p_2$

This family $\mathcal{H}$ is referred to as the LSH family for the distance measure $D$. To improve performance, it is desirable to amplify the gap between $o_1$ and $o_2$, which can be achieved by concatenating multiple LSH functions. Specifically, for a given $k$, define a new family of function $\mathcal{G}$ such that for each $x$, the function $g(o)$ is defined as:

$$
g(o)=(h_1(o), h_2(o), h_3(o),\cdots,h_k(o))
$$
...
To process a query, we search through the buckets correspondent to $g_1(q),g_2(q),\cdots,g_L(q)$, retrieve the points (including duplicates) from the each bucket until one of the following conditions is met:

- All points from the $L$ buckets have been retrieved, or

- The total number of points retrieved exceeds $3L$.

Let $o_1, o_2, ..., o_t$ be the points encountered during this process. If there exists a point $o_i$ such that $\text{dist}(o_i,q)\le r_2=c\cdot R$, then we return YES along with the point $o_i$. Otherwise, we return NO.

Observer that if the following two events hold, the algorithm is correct:

- $E_1$: If there exists $o_i\in\mathcal{O}$ such that $\mathrm{dist}(o_i,q)\le R$, then $g_j(o_i)=g_j(q)$ for some $j\in\{1,\cdots,L\}$.
- $E_2$: The total number of elements $o\in\mathcal{O}$ such that $g_j(o)=g_j(q)$ and $\mathrm{dist}(o,q)>cR$ is at most $3L$.
...
Set $k=\lceil\log_{1/p_2}n\rceil$, where $n=\left|\mathcal{O}\right|$. For fix $j$, the probability that $g_j(o_i)=g_j(q)$ is at most:

$$
p_2^k=p_2^{\lceil\log_{1/p_2}n\rceil}\le p_2^{\log_{1/p_2}n}=\frac{1}{n}
$$

Therefore, the expect number of the points satisfying $\text{dist}(o_i,q)$ that collide with $q$ via $g_j$ is at most $1$. Since there are $L$ hash functions, by Markov's inequality, the probability that the number of collisions exceeds $3L$ is at most 
...
If there exists an $o_i\in\mathcal{O}$ such that  $\mathrm{dist}(o_i,q)\le R$, for a fixed $j$, we have:

$$
\Pr[g_j(o_i)=g_j(q)]=p_1^k
$$
...
let $R_0$ be the smallest **possible** distance between $q$ and a point in $\mathcal{O}$. For example, in the case of Hamming distance, the smallest possible distance is 1. Then define the set of radii:
...
where $o^\ast$ is the exact nearest neighbor of $q$. By the definition of the $(R,c)$-NNS, the algorithm must return a point $\tilde{o}$ such that:

$$
\text{dist}(\tilde{o},q)\le c^{i+2}R_0=c^2\cdot c^iR_0\lt c^2\text{dist}(o^\ast,q)
$$

Thus, $\tilde{o}$ is the $c^2$-NN of the $q$.